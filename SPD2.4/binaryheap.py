...
class BinaryMinHeap:
    ...
    def _bubble_down(self, index):
        ...
        child_index = left_index if right_index >= self.size() or self.items[left_index][1] <= self.items[right_index][1] else right_index

        if item[1] >= self.items[child_index][1]:
